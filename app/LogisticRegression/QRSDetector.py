...
import scipy.io, csv, statistics, math

ecg_measurements = []
inputData = []
countDiscarded = 0
#Features are: average, squareAvg, variance, math.sqrt(variance), median, avgSquared, medianSquared, (avgSquared + medianSquared)/2, mode^2"

class QRSDetectorOffline(object):
	def __init__(self, ecg_data_path, fileNumber, label):
		"""
		...
		self.integration_window = 18

		self.findpeaks_limit = 0.35
		#Change proportionally when adjusting frequency (in samples)
		self.findpeaks_spacing = 60
		
		#Change proportionally when adjusting frequency (in samples)
		self.refractory_period = 144
		...
		response = self.print_detection_data()
		if (len(response) != 2):
			inputData.append([self.fileNumber, response[0], response[1], response[2], response[3], response[4],
				response[5], response[6], response[7], response[8], self.label])

	"""Loading ECG measurements data methods."""
	def load_ecg_data(self):
		"""Method loading ECG data set from a file."""
		mat = scipy.io.loadmat(self.ecg_data_path)
		self.ecg_data_raw = np.array([[i] for i in mat['val'][0]])

	"""ECG measurements data processing methods."""
	def detect_peaks(self):
		"""Method responsible for extracting peaks from loaded ECG measurements data through measurements processing."""
		# Extract measurements from loaded ECG data.
		global ecg_measurements
		mat = scipy.io.loadmat(self.ecg_data_path)
		ecg_measurements = mat['val'][0]

		# Measurements filtering - 0-15 Hz band pass filter.
		self.filtered_ecg_measurements = self.bandpass_filter(ecg_measurements, lowcut=self.filter_lowcut,
			highcut=self.filter_highcut, signal_freq=self.signal_frequency, filter_order=self.filter_order)
		...
		self.detected_peaks_indices = self.findpeaks(data=self.integrated_ecg_measurements, limit=self.findpeaks_limit,
	...
	def detect_qrs(self):
		...
		measurement_qrs_detection_flag = np.zeros([len(ecg_measurements), 1])
	...
	def print_detection_data(self):
		...
		hrList = []
		if (len(self.qrs_peaks_indices) > 5):
			average = 0
			squareAvg = 0
			cubeAvg = 0
			for i in range(1,len(self.qrs_peaks_indices)):
				value = (self.qrs_peaks_indices[i] - self.qrs_peaks_indices[i-1]) / self.signal_frequency
				hrList.append(value)
				average += value
				squareAvg += value**2

			average /= len(self.qrs_peaks_indices) - 1
			squareAvg /= len(self.qrs_peaks_indices) - 1
			median = statistics.median(hrList)
			numeral=[[hrList.count(nb), nb] for nb in hrList]
			...
			variance /= len(self.qrs_peaks_indices) - 1
			avgSquared = average ** 2
			medianSquared = median ** 2

			if (variance != 0):
				return average, squareAvg, variance, math.sqrt(variance), median, avgSquared, medianSquared, (avgSquared + medianSquared)/2, mode**2
		else:
			global countDiscarded
			countDiscarded +=1
		return -1, -1
...
if __name__ == "__main__":
	with open('datasets/originalDataset.csv', mode='r') as csv_file:
		csv_reader = csv.reader(csv_file)
		for row in csv_reader:
			qrs_detector = QRSDetectorOffline(ecg_data_path='training/' + row[0] + '.mat', fileNumber=row[0], label=row[1])

	print(str(countDiscarded) + " training samples were discarded.")
	with open('datasets/Dataset_withFeatures.csv', 'w') as csvfile:
