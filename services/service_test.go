...
func TestServiceLeMal(t *testing.T) {
	...
	for i, op := range operationList {
		...
		maxGenerateData := 4
		dimensions := 5
		operation := lib.ChooseOperation(op, minGenerateData, maxGenerateData, dimensions, cuttingFactor)
		...
		diffP := lib.QueryDiffP{}
		if diffPri {
			diffP = lib.QueryDiffP{LapMean: 0, LapScale: 15.0, NoiseListSize: 1000, Limit: 65, Scale: 1}
		} else {
			diffP = lib.QueryDiffP{LapMean: 0.0, LapScale: 0.0, NoiseListSize: 0, Quanta: 0.0, Scale: 0}
		}
		...
		if !lib.CheckParameters(sq, diffPri) {
			log.Fatal("Oups!")
		}
	...
	if proofs != 0 {
		...
		// close DB
		clientSkip.SendCloseDB(elVNs, &lib.CloseDB{Close: 1})
	}
}
...
func TestServiceLeMalLogisticRegressionForSPECTF(t *testing.T) {
	...
	dataset := "SPECTF"
	lrParameters := lib.LogisticRegressionParameters{K: 2, PrecisionApproxCoefficients: scale, Lambda: 1.0, Step: 0.012, MaxIterations: 450,
	...
	operation := lib.Operation{NameOp: "logistic regression", LRParameters: lrParameters}
	...
	dpData := lib.QueryDPDataGen{GroupByValues: []int64{}, GenerateRows: 100, GenerateDataMin: minGenerateData, GenerateDataMax: maxGenerateData}
...
func TestServiceLeMalLogisticRegression(t *testing.T) {
	...
	dataset := "PCS"
	ratio := 0.8
	lrParameters := lib.LogisticRegressionParameters{K: 2, PrecisionApproxCoefficients: scale, Lambda: 1.0, Step: 0.1, MaxIterations: 25,
